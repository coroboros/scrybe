...
/// The `Agents:` footer under the `--help` flag list: how an AI agent loads
/// scrybe's bundled skill or installs it. The `skills get` command prints the same
/// skill that `npx skills add` installs.
const AGENTS_HELP: &str = "\
Agents:
  scrybe skills get scrybe         print the bundled CLI skill to stdout
  npx skills add coroboros/scrybe  install the skill into your agent";

/// scrybe — collapse sound into signal. Transcribe audio to text, offline.
#[derive(Debug, Parser)]
#[command(name = "scrybe", version, about, long_about = None, propagate_version = true, after_help = AGENTS_HELP)]
pub struct Cli {
...
pub enum Command {
    ...
    /// Print or list the agent skills bundled with scrybe.
    Skills {
        #[command(subcommand)]
        action: SkillsAction,
    },
...
/// Actions under `scrybe skills`. The binary ships a skill for AI agents; install
/// it the proper way with `npx skills add coroboros/scrybe`, or read it inline here.
#[derive(Debug, Subcommand)]
pub enum SkillsAction {
    /// List the agent skills bundled with the binary.
    List,
    /// Print a bundled skill's Markdown to stdout (defaults to `scrybe`).
    Get {
        /// Skill name; defaults to scrybe's own usage guide.
        name: Option<String>,
    },
}
